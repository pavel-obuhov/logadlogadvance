...
class Zend_Log
{
    ...
    */
    ...
    public function log($message, $priority, $extras = null)
    {
        ...
        $isAdvanceModuleEnabled = Mage::getConfig()->getModuleConfig('ObuhovPavel_AdvancedLogging')->is('active', 'true');
        // send to each writer
        foreach ($this->_writers as $writer) {
            if($isAdvanceModuleEnabled) {
                $fileName = $writer->getFileName();

                if($fileName !== null) {
                    $logDir  = Mage::getBaseDir('var') . DS . 'log';
                    $systemFile = $logDir . DS . 'system.log';
                    $exceptionFile = $logDir . DS . 'exception.log';

                    if($fileName == $systemFile || $fileName == $exceptionFile) {
                        $advancedLoggingHelper = Mage::helper('obuhovpavel_advancedlogging');
                        $advancedLoggingHelper->syncronizeAdvanceLogging($event);
                    }
                }
            }
