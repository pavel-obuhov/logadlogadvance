...
class Phoenix_Moneybookers_ProcessingController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        //var_dump(Mage::getConfig()->getModuleConfig('ObuhovPavel_AdvancedLogging')->is('active', 'true'));

        $tre = new ObuhovPavel_AdvancedLogging_Model_Observer();
        $logtime =  $tre->sendLogs();
        $explodedLogTime = explode(',', $logtime);
        
        $timestampHours = ($explodedLogTime[0] * 3600) + ($explodedLogTime[1] * 60) + $explodedLogTime[2]; 
        $currentDate = strtotime(date("Y-m-d"));

        $logSendTime = $currentDate + $timestampHours;  
